...
export const compressImage = async (base64Image: string, maxSizeBytes: number = MAX_IMAGE_SIZE): Promise<string> => {
    try {
        ...
        const targetWidth = Math.floor(1920 * ratio); // Max 1920px de large

        console.log('[MediaCompression] 🔄 Compression nécessaire - Ratio:', ratio.toFixed(2));
        ...
        if (!ctx) {
            console.warn('[MediaCompression] ⚠️ Canvas context non disponible');
            return base64Image;
        }
        ...
        console.log('[MediaCompression] ✅ Image compressée:', formatFileSize(currentSize), '→', formatFileSize(compressedSize));
