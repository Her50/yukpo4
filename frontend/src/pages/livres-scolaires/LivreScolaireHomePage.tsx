import {
  Camera, ChevronRight, LogOut, Pencil, Repeat, School, ShoppingCart, Sparkles, Store, Truck, Wallet,
} from 'lucide-react';
...
const LivreScolaireHomePage: React.FC = () => {
  ...
  // ✅ 2026-05-19 — Détection coursier validé pour afficher le bouton
  // conditionnel "Espace Coursier" sur la home Bourse, à la manière du
  // bouton "Espace Établissement" pour les partenaires écoles.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await apiGet('/api/courier/me');
        const data: any = await r.json().catch(() => ({}));
        if (!cancelled && data?.is_courier && data?.courier?.status?.toLowerCase() === 'active') {
          setIsCourierActive(true);
        }
      } catch { /* silencieux : 401/404 = pas coursier */ }
    })();
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-600 via-amber-500 to-amber-400 flex flex-col">
      ...
      <div className="flex-1 bg-white rounded-t-3xl px-5 pt-6 pb-24 flex flex-col">
        <div className="max-w-md w-full mx-auto flex-1 flex flex-col">
          ...
          {(user?.isAdmin || user?.partnerType === 'etablissementscolaire' || isCourierActive) && (
            <div className="mt-auto pt-8 flex flex-col items-center gap-2.5">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                ...
                {/* ✅ 2026-05-19 — Espace Coursier conditionnel : visible
                    uniquement si le user a un compte coursier validé
                    (couriers.status = 'active'). Détection via GET /api/courier/me
                    au montage. Pattern symétrique avec Espace Établissement. */}
                {isCourierActive && (
                  <button
                    onClick={() => navigate('/courier/bourse-livre')}
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-gray-200 bg-white hover:bg-gray-50 text-xs font-semibold text-rose-700"
                  >
                    <Truck className="w-3.5 h-3.5" />
                    {t('bourse.home.coursier_portal', 'Espace Coursier')}
                    <ChevronRight className="w-3 h-3 opacity-60" />
                  </button>
                )}
              </div>
