...
export const productsService = {
  ...
  getProduct: async (serviceId: number, productIndex: number): Promise<Product> => {
    const response = await apiGet(`/api/services/${serviceId}/products/${productIndex}`);
    const data = await response.json();
    return data as Product;
  },
  ...
  updateProduct: async (
    ...
    const response = await apiPatch(`/api/services/${serviceId}/products/${productIndex}`, {
  ...
  duplicateProduct: async (serviceId: number, productIndex: number): Promise<Product> => {
    const response = await apiPost(`/api/services/${serviceId}/products/${productIndex}/duplicate`, {});
    const data = await response.json();
    return data as Product;
  },
  ...
  getProductsByUser: async (userId: number): Promise<Product[]> => {
    const response = await apiGet(`/api/products?user_id=${userId}`);
    const data = await response.json();
    return data as Product[];
  },
};
