const secondsToFrames = (seconds, fallback = 4) => Math.max(30, Math.round((seconds ?? fallback) * 30));
const baseScene = (id, template, durationSeconds, assets, transition = {
    type: 'hard-cut',
    durationInFrames: 18
}) => ({
    id,
...
const storyTemplates = {
    blog: {
        ...
        description: 'Idéal pour des contenus éditoriaux, annonces de nouveautés ou récaps hebdo.',
    ...
    tutorial: {
        ...
        blueprint: (ctx) => {
            const steps = ctx.tutorialSteps && ctx.tutorialSteps.length > 0
                ? ctx.tutorialSteps
            ...
            return [
                ...
                ...steps.slice(0, 3).map((step, index) => baseScene(`tuto-step-${index + 1}`, 'ProductShowcase', 8, {
    ...
    testimonial: {
        ...
        description: 'Renforce la preuve sociale avec citation, métriques et CTA de confiance.',
        ...
        blueprint: (ctx) => {
            const quote = ctx.testimonialQuote ??
    ...
    comparison: {
        ...
        description: 'Oppose deux options (avant/après, DIY vs service Yukpo) pour générer l’action.',
...
export const STORY_TEMPLATE_IDS = Object.keys(storyTemplates);
export const pickTemplateForContext = (ctx) => {
    ...
    const scored = STORY_TEMPLATE_IDS.map((id) => {
        ...
        if (category &&
            spec.recommendedCategories.some((value) => value.toLowerCase().includes(category))) {
            score += 2;
        }
        if (cta &&
    ...
    return best && best.score > 0 ? best.id : 'blog';
};
export const buildTimelineFromTemplate = (templateId, ctx = {}, overrides) => {
