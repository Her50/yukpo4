...
import { createStorageUploader } from './storage/s3.js';
...
app.post('/render', async (req, res) => {
    const { job_id: jobId, timeline: rawTimeline, story_template_id: storyTemplateId, story_context: storyContext, timeline_overrides: timelineOverrides } = req.body ?? {};
    ...
    if (!timeline && storyTemplateId) {
        ...
        const context = (typeof storyContext === 'object' && storyContext !== null
            ? storyContext
            : {});
        const overrides = typeof timelineOverrides === 'object' && timelineOverrides !== null
            ? timelineOverrides
            : undefined;
...
app.listen(PORT, () => {
    console.log(pc.green(`[video-renderer] 🚀 Serveur RPC prêt sur le port ${PORT} (mode GPU=${process.env.VIDEO_RENDERER_ENABLE_GPU ?? 'false'})`));
});
