...
export const isS3UploadEnabled = () => {
    return (envBool('RENDERER_S3_UPLOAD', false) ||
...
export const loadStorageConfig = () => {
    ...
    const storagePrefix = env('S3_STORAGE_PREFIX') ??
        env('MEDIA_STORAGE_PREFIX') ??
        'uploads';
    const uploadPrefix = env('S3_RENDER_UPLOAD_PREFIX') ??
        env('VIDEO_RENDERER_UPLOAD_PREFIX') ??
        'renders';
    const publicBaseUrl = env('UPLOAD_BASE_URL') ??
        env('PUBLIC_BASE_URL');
    return {
        bucket,
        region,
...
export const buildS3ClientConfig = (config) => {
    const clientConfig = {};
    if (config.region) {
