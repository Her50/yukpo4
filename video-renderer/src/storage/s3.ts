...
export type UploadArtifactsResult = {
    video?: UploadedArtifact;
    timeline?: UploadedArtifact;
    warnings: string[];
};

export type StorageUploader = {
    uploadRenderedArtifacts: (result: RenderResult) => Promise<UploadArtifactsResult>;
};

const buildPublicBaseUrl = (config: RendererStorageConfig): string => {
    ...
    if (config.endpoint) {
        try {
            ...
            console.warn(
                pc.yellow(
                    `[video-renderer] Impossible de parser S3_ENDPOINT (${config.endpoint}): ${String(
                        error
                    )}`
                )
            );
...
export const createStorageUploader = (): StorageUploader | null => {
    ...
    if (!config) {
        console.warn(
            pc.yellow(
                '[video-renderer] RENDERER_S3_UPLOAD activé mais configuration S3 incomplète.'
            )
        );
        return null;
    }
    ...
    const uploadRenderedArtifacts = async (
        result: RenderResult
        ...
        let videoArtifact: UploadedArtifact | undefined;
        ...
        try {
            ...
            console.log(
                pc.green(
                    `[video-renderer] ✅ Vidéo uploadée vers S3 (${videoStoragePath})`
                )
            );
        } catch (error) {
        ...
        const timelineArtifact = await (async (): Promise<UploadedArtifact | undefined> => {
            try {
                ...
                console.log(
                    pc.green(
                        `[video-renderer] ✅ Timeline uploadée vers S3 (${timelineStoragePath})`
                    )
                );
                ...
                const message = `[video-renderer] ⚠️ Upload S3 timeline échoué: ${String(
                    error
                )}`;
