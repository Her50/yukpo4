...
Write-Host "Test de la recherche apres correction" -ForegroundColor Green
...
try {
    ...
    if ($response.resultats -and $response.resultats.Count -gt 0) {
        ...
        $response.resultats | ForEach-Object { 
            Write-Host "- $($_.titre) (score: $($_.similarity_score))" -ForegroundColor Yellow 
        }
    } else {
