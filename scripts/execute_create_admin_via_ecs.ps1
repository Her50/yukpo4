# Script pour creer le compte SUPER SUPER ADMIN via ECS Task
# Email: admin@yukpo.dev
# Mot de passe: Hernandez87
# Role: super_admin (tous les droits)
# Usage: .\scripts\execute_create_admin_via_ecs.ps1

param(
    [switch]$AutoConfirm = $false
)

Write-Host "[ADMIN] Creation du compte SUPER SUPER ADMIN via ECS Task" -ForegroundColor Green
...
if (-not (Test-Path $sqlScriptPath)) {
    Write-Host "[ERROR] Script SQL non trouve: $sqlScriptPath" -ForegroundColor Red
    exit 1
}

# Lire le script SQL
$sqlScript = Get-Content $sqlScriptPath -Raw -Encoding UTF8
Write-Host "[OK] Script SQL charge: $sqlScriptPath" -ForegroundColor Green
...
$TASK_DEFINITION = "$TASK_DEFINITION_FAMILY:4"  # Version par defaut

# Essayer de recuperer la derniere version
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $taskDefOutput) {
        # Extraire uniquement la ligne qui contient l'ARN
...
Write-Host "[INFO] Encodage du script SQL en base64..." -ForegroundColor Yellow
...
# Utiliser des guillemets simples pour eviter les problemes d'echappement
$command = "echo '$sqlBase64' | base64 -d | psql `$DATABASE_URL"

# Creer les overrides JSON avec ConvertTo-Json
$overrides = @{
    containerOverrides = @(
        @{
            name = $CONTAINER_NAME
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "Ou pour cette task specifique:" -ForegroundColor Cyan
        Write-Host "   aws logs filter-log-events --log-group-name /ecs/yukpomnang-backend --region $REGION --filter-pattern `"$taskId`"" -ForegroundColor White
        Write-Host ""
