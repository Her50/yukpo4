# Script pour tester l'exécutable dans une tâche ECS
# Vérifie que l'exécutable existe, ses permissions, et ses dépendances
...
Write-Host "  TEST 1: VERIFICATION DE L'EXECUTABLE" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Récupérer les informations du réseau depuis la task definition
...
Write-Host "Network Mode: $networkMode" -ForegroundColor Gray
Write-Host "Requires Compatibilities: $($requiresCompatibilities -join ', ')" -ForegroundColor Gray
Write-Host ""

# Pour Fargate, on a besoin des subnets et security groups
# Récupérer depuis une tâche existante ou utiliser des valeurs par défaut
Write-Host "Recuperation des subnets et security groups..." -ForegroundColor Cyan
...
if ($runningTasks.taskArns -and $runningTasks.taskArns.Count -gt 0) {
    ...
    if (-not $securityGroupIds) {
        $securityGroupIds = @()
        $securityGroupAttachments = $attachments | Where-Object { $_.name -eq "securityGroupIds" }
        if ($securityGroupAttachments) {
            $securityGroupIds = @($securityGroupAttachments.value)
        }
    }
    Write-Host "  Subnet: $subnetId" -ForegroundColor Green
    Write-Host "  Security Groups: $($securityGroupIds -join ', ')" -ForegroundColor Green
} else {
    Write-Host "  ⚠️ Aucune tache en cours, recuperation depuis la task definition..." -ForegroundColor Yellow
    # Récupérer depuis la task definition ou utiliser des valeurs par défaut
    $subnetId = "subnet-0bdead65f27d8039c"  # Subnet trouvé précédemment
    $securityGroupIds = @("sg-0d910f6cca6bac2e5")  # Security group ECS
    Write-Host "  Subnet: $subnetId" -ForegroundColor Yellow
    Write-Host "  Security Groups: $($securityGroupIds -join ', ')" -ForegroundColor Yellow
}

Write-Host ""
Write-Host "Lancement de la tache de test..." -ForegroundColor Cyan

$overrides = @{
    containerOverrides = @(
        @{
            name = "backend"
            command = @("sh", "-c", "echo '=== TEST 1: Verification executable ===' && ls -la /app/yukpomnang_backend && echo '' && echo '=== Type de fichier ===' && file /app/yukpomnang_backend && echo '' && echo '=== Permissions ===' && stat /app/yukpomnang_backend 2>/dev/null || ls -l /app/yukpomnang_backend")
        }
    )
...
if (-not $taskResult.tasks -or $taskResult.tasks.Count -eq 0) {
    ...
    Write-Host "Reponse: $($taskResult | ConvertTo-Json -Depth 10)" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  TEST 1 TERMINE" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
