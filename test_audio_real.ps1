# Test de transcription audio avec fichier realiste
Write-Host "Test de transcription audio avec fichier realiste" -ForegroundColor Green
...
# Creer un fichier audio plus realiste (avec silence)
Write-Host "`nCreation d'un fichier audio de test (silence)..." -ForegroundColor Cyan
...
$wavHeader = @(
    ...
    0x24, 0x10, 0x00, 0x00,  # Taille du fichier - 8 (corrigee)
    ...
    0x00, 0x10, 0x00, 0x00   # Taille des donnees (corrigee)
)

# Donnees audio (silence)
$audioData = @()
for ($i = 0; $i -lt $samples; $i++) {
    $audioData += 0x00, 0x00  # Silence (16-bit)
}
...
$tempAudioPath = "temp_silence_audio.wav"
[System.IO.File]::WriteAllBytes($tempAudioPath, $wavBytes)

Write-Host "Fichier audio cree: $tempAudioPath" -ForegroundColor Green

# Test avec l'API OpenAI Whisper
Write-Host "`nTest avec l'API OpenAI Whisper..." -ForegroundColor Cyan
...
Write-Host "Envoi de la requete a OpenAI Whisper..." -ForegroundColor Yellow
...
try {
    ...
    if ($response.text -eq "") {
        Write-Host "Transcription vide (normal pour un fichier silence)" -ForegroundColor Yellow
    } else {
        Write-Host "Transcription reussie!" -ForegroundColor Green
    }
} catch {
