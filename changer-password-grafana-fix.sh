#!/bin/bash
# Script pour changer le mot de passe Grafana via API (version fixée)
...
if [ -z "$NEW_PASSWORD" ]; then
    echo "Usage: GRAFANA_NEW_PASSWORD='votre_mot_de_passe' bash changer-password-grafana-fix.sh"
    exit 1
fi
