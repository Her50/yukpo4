Write-Host "Debug de la requête backend vers microservice" -ForegroundColor Cyan

# Simuler exactement ce que le backend envoie
...
# Requête exacte que le backend envoie
$backendRequest = @{
...
try {
    ...
    if ($response.results.Count -gt 0) {
        Write-Host "`nPremiers résultats:" -ForegroundColor Green
...
Write-Host "`n2. Test avec requête différente..." -ForegroundColor Yellow
...
try {
    $response2 = Invoke-RestMethod -Uri "http://localhost:8000/search_embedding_pinecone" -Method POST -Headers $headers -Body $testRequest -TimeoutSec 30
