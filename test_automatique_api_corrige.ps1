...
Write-Host "`nTest 1: Service tarissable avec produits" -ForegroundColor Magenta
$test1Data = @{
...
if ($success1) {
    ...
    Write-Host "`nAnalyse de la reponse Test 1:" -ForegroundColor Blue
...
Write-Host "`nTest 2: Service avec GPS fixe" -ForegroundColor Magenta
$test2Data = @{
...
if ($success2) {
    Write-Host "`nAnalyse de la reponse Test 2:" -ForegroundColor Blue
...
Write-Host "`nTest 3: Service avec donnees modales" -ForegroundColor Magenta
$test3Data = @{
...
if ($success3) {
    Write-Host "`nAnalyse de la reponse Test 3:" -ForegroundColor Blue
...
if ($success1) {
    Write-Host "`nTest 4: Creation de service en base" -ForegroundColor Magenta
    ...
    if ($success4) {
        Write-Host "`nAnalyse de la creation:" -ForegroundColor Blue
        if ($response4.id) {
...
Write-Host "===================" -ForegroundColor Green
Write-Host "Test 1 (Tarissable): $(if($success1) {'OK'} else {'ECHEC'})" -ForegroundColor $(if($success1) {'Green'} else {'Red'})
Write-Host "Test 2 (GPS fixe): $(if($success2) {'OK'} else {'ECHEC'})" -ForegroundColor $(if($success2) {'Green'} else {'Red'})
Write-Host "Test 3 (Modales): $(if($success3) {'OK'} else {'ECHEC'})" -ForegroundColor $(if($success3) {'Green'} else {'Red'})
Write-Host "Test 4 (Creation): $(if($success4) {'OK'} else {'ECHEC'})" -ForegroundColor $(if($success4) {'Green'} else {'Red'})

$totalTests = 4
$passedTests = @($success1, $success2, $success3, $success4) | Where-Object { $_ } | Measure-Object | Select-Object -ExpandProperty Count

Write-Host "`nResultat: $passedTests/$totalTests tests reussis" -ForegroundColor $(if($passedTests -eq $totalTests) {'Green'} else {'Yellow'})

if ($passedTests -eq $totalTests) {
    Write-Host "Tous les tests sont passes ! Le systeme fonctionne correctement." -ForegroundColor Green
} else {
    Write-Host "Certains tests ont echoue. Verifiez les logs ci-dessus." -ForegroundColor Yellow
} 