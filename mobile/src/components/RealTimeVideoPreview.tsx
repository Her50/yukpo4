...
export const RealTimeVideoPreview: React.FC<RealTimeVideoPreviewProps> = ({
    ...
    // Charger la vidéo et générer les previews
    useEffect(() => {
        loadVideo();
    }, [videoUrl]);
    ...
    const loadVideo = useCallback(async () => {
        ...
        try {
            ...
            console.error('[RealTimeVideoPreview] Erreur chargement vidéo:', err);
    ...
    const generatePreviewFrames = useCallback(async (videoDuration: number) => {
        ...
        try {
            ...
            console.warn('[RealTimeVideoPreview] Erreur génération frames:', err);
            // Ne pas bloquer si les frames ne peuvent pas être générées
        } finally {
    ...
    const handleError = useCallback((error: any) => {
        console.error('[RealTimeVideoPreview] Erreur lecture vidéo:', error);
    ...
    if (isLoading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                {isGeneratingPreview && (
                    <Text style={styles.generatingText}>Génération des previews...</Text>
                )}
            </View>
        );
    }
