...
const Dashboard: React.FC = () => {
  ...
  return (
    <ResponsiveContainer>
      ...
      {user && !loading && (
        <View style="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <View style="flex items-center justify-between">
            <View style="flex items-center space-x-2">
              <List style="w-5 h-5 text-blue-600" />
              <Text style="text-blue-800 font-medium">Gestion de vos services</Text>
            </View>
            <Link
              to={ROUTES.MES_SERVICES}
              style="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center space-x-2"
            >
              <List style="w-4 h-4" />
      ...
      {process.env.NODE_ENV === 'development' && (
        <View style="mb-4 p-2 bg-gray-100 rounded text-xs">
          <Text>Debug: user={!!user}, loading={loading}, hasServices={hasServices}</Text>
          <Text>Condition: {user && !loading && hasServices ? 'TRUE' : 'FALSE'}</Text>
        </View>
      )}
      ...
      <ul style="space-y-2">
        <RequireAccess plan="enterprise">
          <li>
            <Link to={ROUTES.DASHBOARD} style="text-blue-600 underline">
