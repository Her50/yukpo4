...
const BanqueSangFormScreen: React.FC = () => {
    ...
    const handleUpdateStocks = async () => {
        ...
        setLoading(true);
        try {
            await apiPost(`/api/banques-sang/${bankData.id}/stocks`, { stocks_groupes_sanguins: stocks });
    ...
    if (initialLoading) return <View style={s.loadingScreen}><ActivityIndicator size="large" color="#DC2626" /><Text style={s.loadingText}>{t('banqueSangForm.chargement') || 'Chargement...'}</Text></View>;
    ...
    const renderOverview = () => (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}
            ...
            <View style={s.statsGrid}>
                {[
                    { label: t('banqueSangForm.groupesDispo') || 'Groupes dispo', value: groupsWithStock, icon: 'droplets', color: '#DC2626' },
                    { label: t('banqueSangForm.stockTotal') || 'Stock total', value: `${totalStock} u.`, icon: 'package', color: '#3B82F6' },
                    { label: t('banqueSangForm.dons') || 'Dons', value: formData.accepte_dons ? '✓' : '✗', icon: 'heart', color: '#10B981' },
                ].map((st, i) => (
            ...
            <View style={[s.urgenceCard, { backgroundColor: formData.urgence_24h ? '#FEF2F2' : '#F3F4F6' }]}>
                ...
                <Text style={[s.urgenceText, { color: formData.urgence_24h ? '#DC2626' : '#6B7280' }]}>
                    {formData.urgence_24h ? (t('banqueSangForm.urgence24hActive') || 'Service d\'urgence 24h activé') : (t('banqueSangForm.urgenceDesactive') || 'Service d\'urgence désactivé')}
                </Text>
            </View>
            ...
            <View style={s.quickRow}>
                <TouchableOpacity style={s.quickAction} onPress={() => setActiveTab('stocks')}>
                    ...
                    <Text style={s.quickLabel}>{t('banqueSangForm.gererStocks') || 'Gérer stocks'}</Text>
                </TouchableOpacity>
            ...
            {groupsWithStock > 0 && (
                <>
                    <Text style={s.sectionTitle}>{t('banqueSangForm.stocksDisponibles') || 'Stocks disponibles'}</Text>
            ...
            {statistics && (
                <>
                    <Text style={[s.sectionTitle, { marginTop: 16 }]}>{t('banqueSangForm.statistiques') || 'Statistiques'}</Text>
                    {[
                        statistics.total_donations !== undefined && { l: t('banqueSangForm.donsRecus') || 'Dons reçus', v: statistics.total_donations },
                        statistics.total_requests !== undefined && { l: t('banqueSangForm.demandesTraitees') || 'Demandes traitées', v: statistics.total_requests },
                        statistics.urgent_requests !== undefined && { l: t('banqueSangForm.urgences') || 'Urgences', v: statistics.urgent_requests },
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, padding: 16 }}>
            ...
            <View style={s.field}>
                <TouchableOpacity style={s.gpsBtn} onPress={() => setShowGPSModal(true)}>
                    ...
                    <Text style={s.gpsBtnText}>{selectedGPS ? ('✓ ' + (t('banqueSangForm.gpsSelectionne') || 'GPS sélectionné')) : (t('banqueSangForm.selectionnerCarte') || 'Sélectionner sur la carte')}</Text>
            ...
            <NativeButton title={loading ? (t('banqueSangForm.enregistrementEnCours') || 'Enregistrement...') : (isDashboardMode ? (t('banqueSangForm.mettreAJour') || 'Mettre à jour') : (t('banqueSangForm.enregistrer') || 'Enregistrer'))} onPress={handleSubmit} disabled={loading || !formData.nom.trim()} variant="primary" size="large" style={{ marginTop: 24 }} />
        </ScrollView>
    );
    ...
    const renderStocksTab = () => (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, padding: 16 }}>
            <View style={s.stockHeader}>
                <Text style={s.sectionTitle}>{t('banqueSangForm.gestionStocks') || 'Gestion des stocks'}</Text>
                <Text style={s.stockSummary}>{groupsWithStock}/{GROUPES_SANGUINS.length} {t('banqueSangForm.groupes') || 'groupes'} · {totalStock} {t('banqueSangForm.unites') || 'unités'}</Text>
            </View>
            ...
            <NativeButton title={loading ? (t('banqueSangForm.miseAJourEnCours') || 'Mise à jour...') : (t('banqueSangForm.mettreAJourStocks') || 'Mettre à jour les stocks')} onPress={handleUpdateStocks} disabled={loading} variant="primary" size="large" style={{ marginTop: 24 }} />
        </ScrollView>
    );
    ...
    if (isDashboardMode || (user?.role === 'partenaire' && serviceId)) {
        const tabs: { key: TabType; label: string; icon: string }[] = [
            { key: 'overview', label: t('banqueSangForm.accueil') || 'Accueil', icon: 'layout-dashboard' },
            { key: 'service', label: t('banqueSangForm.service') || 'Service', icon: 'settings' },
            { key: 'stocks', label: t('banqueSangForm.stocks') || 'Stocks', icon: 'droplets' },
        ];
        return (
