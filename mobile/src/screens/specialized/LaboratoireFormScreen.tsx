...
const LaboratoireFormScreen: React.FC = () => {
    ...
    useEffect(() => {
        const loadPartnerData = async () => {
            if (user?.role === 'partenaire' && user?.partner_type === 'laboratoire' && partnerData) {
                try {
...
const confirmationSections: ConfirmationSection[] = [
    {
        ...
        fields: [
            ...
            { label: 'Type', value: formData.type_laboratoire },
    ...
    },
    {
        ...
        icon: 'clipboard',
        fields: [
            { label: 'Analyses', value: `${selectedAnalyses.length} type(s)` },
            { label: 'Imagerie', value: `${selectedImagerie.length} type(s)` },
            { label: 'Horaires', value: formData.permanent_24h ? '24h/24' : `${formData.heures_ouverture} - ${formData.heures_fermeture}` },
            { label: 'RDV requis', value: formData.rdv_requis, type: 'boolean' as const },
            { label: 'Résultats en ligne', value: formData.resultats_en_ligne, type: 'boolean' as const },
        ],
    },
];
