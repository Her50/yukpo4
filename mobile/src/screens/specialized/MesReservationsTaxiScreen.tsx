/**
 * Écran réservations taxi — liste filtrée, suivi, QR, annulation, rappels
 */
...
const MesReservationsTaxiScreen: React.FC = () => {
    const navigation = useNavigation();
    const { user } = useAuth();
    const { t } = useLanguageSafe();

    const [reservations, setReservations] = useState<TaxiReservation[]>([]);
    ...
    const [ratingModal, setRatingModal] = useState<{ reservationId: number; ratedUserId: number; driverName?: string } | null>(null);

    const loadReservations = useCallback(async () => {
        try {
            const params = statusFilter
                ? `?status=${statusFilter}&service_type=taxi`
                : '?service_type=taxi';
            const response = await apiGet(`/api/specialized-services/reservations${params}`);
            const resData = (response?.data || response) as any;
            if (resData?.success) {
                const all: TaxiReservation[] = resData.reservations || [];
                setReservations(all.filter((r) => r.service_type === 'taxi'));
            }
        } catch (err: any) {
            console.error('[MesReservationsTaxiScreen] Erreur:', err);
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const handleCancel = async (id: number) => {
        Alert.alert('Annuler le taxi', 'Voulez-vous vraiment annuler cette réservation ?', [
    ...
    const handleScheduleReminders = async (reservation: TaxiReservation) => {
        try {
            ...
            if (data?.success) {
                Alert.alert(
                    'Rappels programmés',
                    `${data.scheduled_count} rappel(s) : 60 min et 15 min avant le départ.`,
                );
            }
        } catch {
    ...
    const handleTrack = (reservation: TaxiReservation) => {
        navigation.navigate('TaxiTracking' as never, {
            taxiId: reservation.service_id,
            bookingId: reservation.id,
        } as never);
    };

    const renderItem = ({ item }: { item: TaxiReservation }) => {
        ...
        const isQrExpanded = expandedQr === item.id;
            ...
            <NativeCard style={styles.card}>
                ...
                <View style={styles.cardHeader}>
                    <View style={styles.cardHeaderLeft}>
                        <SafeIcon name="car" size={18} color="#D97706" />
                        <Text style={styles.serviceLabel}>Taxi</Text>
                    </View>
                ...
                {item.amount != null && item.payment_status !== 'paid' && isActive && (
                    ...
                    >
                        ...
                        <Text style={styles.payBtnText}>Payer {item.amount.toLocaleString()} {item.currency || 'XAF'}</Text>
                    </TouchableOpacity>
                )}
                ...
                {item.status === 'completed' && (
                    <TouchableOpacity
                        ...
                        onPress={() => setRatingModal({
                            ...
                            ratedUserId: (item.details as any)?.driver_user_id || (item.details as any)?.user_id || 0,
                        })}
                    >
                ...
                {(item.status === 'confirmed' || item.status === 'completed') && (
                    <TouchableOpacity
                        ...
                        onPress={() => setExpandedQr(isQrExpanded ? null : item.id)}
                    >
                        <SafeIcon name="qr-code" size={14} color="#6366F1" />
                        <Text style={styles.qrToggleText}>
                            {isQrExpanded ? 'Masquer le QR' : 'Afficher le QR'}
                        </Text>
                        <SafeIcon name={isQrExpanded ? 'chevron-up' : 'chevron-down'} size={14} color="#6366F1" />
                    </TouchableOpacity>
                )}
                {isQrExpanded && <QRCodeDisplay reservationId={item.id} />}
                ...
                {isActive && (
                    <View style={styles.actionsRow}>
                        ...
                        >
                            <SafeIcon name="map-pin" size={14} color="#fff" />
    ...
    const filters: Array<string | null> = [null, 'pending', 'confirmed', 'completed', 'cancelled'];
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <View style={styles.empty}>
                    ...
                    <Text style={styles.emptyText}>Aucune réservation de taxi</Text>
                </View>
            ) : (
                <FlatList
                    data={reservations}
...
const styles = StyleSheet.create({
    ...
    filterChipTextActive: { color: '#D97706', fontWeight: '600' },
    ...
    qrToggleText: { fontSize: 12, color: '#6366F1', fontWeight: '500' },
    actionsRow: {
        flexDirection: 'row',
        marginTop: 10,
        gap: 6,
    ...
    reminderBtnText: { fontSize: 12, color: modernColors.primary, fontWeight: '500' },
    cancelBtn: {
        flex: 1,
        paddingVertical: 8,
        borderRadius: 8,
...
export default MesReservationsTaxiScreen;
