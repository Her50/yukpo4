// @ts-nocheck
// Version restaurée - utilise Alert.alert au lieu de toaster (version stable d'il y a ~1 mois)
...
const MesServicesScreen: React.FC = () => {
  console.log('[MesServicesScreen] 🚀 Démarrage du composant RESTORE');
  
  const navigation = useNavigation();
  const { user } = useAuth();
  const { colors } = useTheme(); // Support thème
  const deviceType = useDeviceType(); // Responsive design
  const { isLandscape } = useDeviceOrientation(); // Orientation
  
  console.log('[MesServicesScreen] ✅ Hooks de base initialisés');
  ...
  // Configuration globale de livraison
  ...
  // Fonction pour parser un produit
  ...
  const extractProduits = useCallback((service: any): any[] => {
    ...
    // Format 5 - produits string à parser
  ...
  const loadServices = useCallback(async (isRefresh = false) => {
    console.log('[MesServicesScreen] 🔍 Début chargement services RESTORE');
    ...
    try {
      ...
      if (response.success) {
        ...
        // Extraire les produits depuis l'API
  ...
  useEffect(() => {
    console.log('[MesServicesScreen] 🚀 Montage RESTORE');
  ...
  }, []); // ✅ CORRECTION : Pas de dépendance à loadServices
  ...
  }, [rawServices, services, navigation, loading]); // ✅ CORRECTION : Retirer loadServices
  ...
  // Filtrer les services selon le statut
  const filteredServices = useMemo(() => {
    if (filter === 'tous') return services;
    return services.filter(service => service.status === filter);
  }, [services, filter]);

  // Rendu simplifié pour le diagnostic
  console.log('[MesServicesScreen] 🎨 Rendu RESTORE - Loading:', loading, 'Services:', services.length);
...
console.log('[MesServicesScreen] ✅ Composant MesServicesScreen RESTORE chargé avec succès');
