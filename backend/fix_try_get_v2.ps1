# Script amélioré pour remplacer try_get par get
...
foreach ($file in $rustFiles) {
    ...
    # Pattern 6: .try_get("col")? -> .get::<_, _>("col") (on laisse le type à corriger manuellement)
    # Échapper le ? avec \? dans le pattern regex
    $content = $content -replace '\.try_get\("([^"]+)")\?', '.get::<_, _>("$1")'
