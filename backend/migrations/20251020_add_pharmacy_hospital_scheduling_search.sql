...
CREATE INDEX IF NOT EXISTS idx_pharmacies_on_duty_status 
...
COMMENT ON FUNCTION is_pharmacy_on_duty(JSONB, TIMESTAMP) IS 
'Vérifie si une pharmacie est de garde à un moment donné en analysant ses jours de garde et heures d''ouverture';

COMMENT ON FUNCTION is_medical_service_available(JSONB, TIMESTAMP, TEXT) IS 
'Vérifie si un service médical est disponible à un moment donné selon son planning hebdomadaire';

COMMENT ON FUNCTION search_products_with_scheduling(TEXT, TIMESTAMP, FLOAT, FLOAT, FLOAT) IS 
