...
-- ============================================================================
-- 1. TABLE : Échanges de livres (historique et tracking) (protégé)
-- ============================================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'livres_scolaires') THEN
        ...
        -- Ajouter la clé étrangère troc_id si la table existe
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'troc_livres_scolaires') THEN
            ALTER TABLE book_exchanges ADD CONSTRAINT book_exchanges_troc_id_fkey 
            FOREIGN KEY (troc_id) REFERENCES troc_livres_scolaires(id) ON DELETE SET NULL;
        END IF;
...
-- ============================================================================
-- 2. TABLE : Recommandations IA de livres (protégé)
-- ============================================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'livres_scolaires') THEN
...
-- ============================================================================
-- 3. TABLE : Historique des prix (pour suggestions prix IA) (protégé)
-- ============================================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'livres_scolaires') THEN
...
-- ============================================================================
-- 4. TABLE : Analytics Bourse du Livre (protégé)
-- ============================================================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'livres_scolaires') THEN
...
-- ============================================================================
-- TRIGGERS POUR UPDATED_AT (protégés)
-- ============================================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'book_exchanges') THEN
        EXECUTE 'COMMENT ON TABLE book_exchanges IS ''Historique et tracking des échanges de livres (troc, achat, vente, don)''';
        EXECUTE 'COMMENT ON COLUMN book_exchanges.exchange_type IS ''Type d''''échange: troc, achat, vente, don''';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'book_recommendations') THEN
        EXECUTE 'COMMENT ON TABLE book_recommendations IS ''Recommandations IA de livres avec scores et reasoning''';
        EXECUTE 'COMMENT ON COLUMN book_recommendations.score_recommendation IS ''Score de recommandation IA (0-100)''';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'book_price_history') THEN
        EXECUTE 'COMMENT ON TABLE book_price_history IS ''Historique des prix pour suggestions prix IA basées sur marché''';
        EXECUTE 'COMMENT ON COLUMN book_price_history.source_type IS ''Source du prix: echange, achat, vente, estimation_ia, marche''';
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'book_analytics') THEN
        EXECUTE 'COMMENT ON TABLE book_analytics IS ''Analytics pour prestataires : vues, contacts, échanges, popularité''';
        EXECUTE 'COMMENT ON COLUMN book_analytics.score_popularite IS ''Score de popularité calculé (0-100)''';
    END IF;
END $$;
