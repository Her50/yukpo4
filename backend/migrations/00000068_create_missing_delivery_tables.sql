...
-- Protection: Créer la vue seulement si la table deliveries existe
DO $$
BEGIN
...
-- Protection: Créer la table courier_profiles avec ou sans FK vers deliveries
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'courier_profiles') THEN
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'deliveries') THEN
            -- Créer avec FK vers deliveries
        ...
        ELSE
            ...
            CREATE TABLE courier_profiles (
                ...
                is_online BOOLEAN DEFAULT FALSE,
                current_status TEXT, -- 'available', 'on_delivery', 'busy', etc.
                current_delivery_id UUID, -- Pas de FK si deliveries n'existe pas
                metadata JSONB DEFAULT '{}'::jsonb,
                created_at TIMESTAMPTZ DEFAULT now(),
                updated_at TIMESTAMPTZ DEFAULT now()
            );
