...
SELECT 
    ...
    -- Pré-calculer le texte de tous les produits (simplifié si fonction n'existe pas)
    COALESCE((
        SELECT string_agg(COALESCE(product->>'nom', product->>'name', ''), ' ')
...
-- Index GIN sur produits (simplifié - pas de sous-requête dans index)
-- Note: Les index avec sous-requêtes SELECT ne sont pas supportés directement
-- Utiliser la vue matérialisée services_search_optimized à la place
...
CREATE INDEX IF NOT EXISTS idx_services_gps_active_category
