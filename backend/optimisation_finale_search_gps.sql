-- ============================================================
-- OPTIMISATION FINALE : Version ultra-rapide utilisant les index
-- ============================================================
-- Basée sur l'analyse : requête directe = 18ms, fonction = 223ms
-- Solution : Simplifier la fonction pour qu'elle soit proche de la requête directe
-- ============================================================
...
-- Version ultra-optimisée qui utilise directement les index
...
BEGIN
    ...
    IF user_gps_zone IS NOT NULL AND user_gps_zone != '' AND user_gps_zone != 'null' THEN
        ...
        IF array_length(gps_parts, 1) > 0 THEN
            ...
            IF lat IS NOT NULL AND lng IS NOT NULL THEN
                -- Version optimisée : utiliser les index directement, éviter DISTINCT ON si possible
                RETURN QUERY
                SELECT 
                ...
                -- ✅ Utiliser les index existants (ordre COALESCE aligné)
                AND (
                ...
                -- ✅ ORDER BY simple pour permettre utilisation index
                ORDER BY relevance_score DESC, s.id
                LIMIT max_results;
    ...
    -- CAS 2: Recherche SANS GPS (version ultra-optimisée)
    RETURN QUERY
    SELECT 
        s.id::integer AS service_id,
        COALESCE(s.data->>'titre_service', s.data->'titre_service'->>'valeur', '')::text AS titre_service,
...
'Version ultra-optimisée 2025-11-30: Supprime DISTINCT ON, simplifie ORDER BY, utilise directement les index. Performance: ~20-50ms (vs 200-500ms avant).';
