...
-- 1. Ajouter product_labels à autocomplete_combinations si elle n'existe pas
DO \$\$ 
BEGIN
    IF NOT EXISTS (
        ...
        COMMENT ON COLUMN autocomplete_combinations.product_labels IS 'Labels correspondant à chaque valeur du product_vector. Ex: [\"marque\", \"modele\", \"couleur\", \"pointure\"]';
    END IF;
END \$\$;

-- 2. Ajouter tokens_ia_consumed à token_usage_logs si elle n'existe pas
DO \$\$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
