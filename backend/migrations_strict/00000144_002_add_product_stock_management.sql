...
-- 3. Table réservations stock (protégée)
-- Protection: Vérifier que la table product_orders existe
DO $$
BEGIN
    ...
    ELSE
        -- Créer sans FK vers product_orders si la table n'existe pas
        CREATE TABLE IF NOT EXISTS stock_reservations (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            order_id UUID NOT NULL, -- Pas de FK si product_orders n'existe pas
            stock_location_id INTEGER NOT NULL REFERENCES product_stock_locations(id),
            quantity INTEGER NOT NULL,
            reserved_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            released_at TIMESTAMPTZ,
