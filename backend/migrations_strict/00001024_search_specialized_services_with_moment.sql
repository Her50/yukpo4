    ...
    WITH laboratory_data AS (
        SELECT 
            ...
            (
                ...
                CASE WHEN EXISTS (
                    SELECT 1 FROM unnest(l.analyses_disponibles) AS analyse_item 
                    WHERE analyse_item ILIKE '%' || search_query || '%'
                ) THEN 7 ELSE 0 END +
                CASE WHEN EXISTS (
...
BEGIN
    ...
    WITH agency_data AS (
        ...
        AND (
            ...
            (current_time_value >= a.heures_ouverture AND current_time_value <= a.heures_fermeture)
        )
    )
    SELECT 
        ad.id,
