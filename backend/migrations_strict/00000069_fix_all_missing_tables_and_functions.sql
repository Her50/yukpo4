...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'services_search_optimized_v2') THEN
        ...
        -- Utiliser EXECUTE pour éviter l'erreur "relation does not exist"
        BEGIN
            EXECUTE 'DROP INDEX IF EXISTS idx_services_search_optimized_v2_unique';
...
-- Commentaire sur l'index (protégé)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM pg_indexes 
