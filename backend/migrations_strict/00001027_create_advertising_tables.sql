...
-- ✅ NOTE 2026-02-15: La table publicite_impressions est déjà créée dans la migration 9
-- Pas besoin de la gérer ici, elle existe déjà avec le bon schéma

-- Table publicites (gestion des publicités)
-- Ajouter la colonne frequency_config si elle n'existe pas déjà
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'publicites')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'publicites' AND column_name = 'frequency_config') THEN
        ALTER TABLE publicites ADD COLUMN frequency_config JSONB DEFAULT '{}';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS publicites (
    ...
    frequency_config JSONB DEFAULT '{}', -- Configuration de fréquence d'affichage (max_per_day, max_per_week, etc.)
...
CREATE INDEX IF NOT EXISTS idx_publicites_frequency_config_gin ON publicites USING GIN(frequency_config);
...
-- ✅ NOTE 2026-02-15: La table publicite_impressions et ses fonctions sont déjà créées dans la migration 9
-- Pas besoin de les recréer ici, elles existent déjà avec le bon schéma
-- On garde seulement la suppression au début pour nettoyer en cas de problème

-- Table pixel_events (tracking pixel pour analytics) (protégée)
DO $$
BEGIN
    -- Supprimer la table existante si elle a un schéma différent
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pixel_events')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'pixel_events' AND column_name = 'event_type') THEN
        DROP TABLE pixel_events CASCADE;
    END IF;
    
    -- Créer la table si elle n'existe pas
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pixel_events') THEN
...
-- Table publicite_audiences (audiences ciblées pour publicités) (protégée)
DO $$
BEGIN
    -- Supprimer la table existante si elle a un schéma différent
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'publicite_audiences')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'publicite_audiences' AND column_name = 'publicite_id') THEN
        DROP TABLE publicite_audiences CASCADE;
    END IF;
    
    -- Créer la table si elle n'existe pas
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'publicite_audiences') THEN
        CREATE TABLE publicite_audiences (
            id SERIAL PRIMARY KEY,
            publicite_id INTEGER NOT NULL REFERENCES publicites(id) ON DELETE CASCADE,
