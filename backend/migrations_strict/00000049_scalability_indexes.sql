...
CREATE INDEX IF NOT EXISTS idx_services_active_category_gps 
ON services (is_active, category, created_at DESC) 
WHERE is_active = TRUE;

-- Index GIN pour recherche full-text dans produits JSON
CREATE INDEX IF NOT EXISTS idx_services_products_fulltext_gin
...
CREATE INDEX IF NOT EXISTS idx_services_products_array_gin
ON services USING GIN (
    (
        CASE 
...
-- Note: Protéger avec DO block car la table peut ne pas exister
DO $$
BEGIN
...
-- Note: CREATE INDEX ne peut pas être dans un DO block
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'delivery_tracking_points') THEN
