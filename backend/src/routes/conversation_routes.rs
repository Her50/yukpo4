...
pub fn conversation_routes(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            ...
            get(get_conversation_participants),
        )
        .route(
            "/api/conversations/{conversation_id}/invite",
            post(invite_user_to_conversation),
        )
        .route(
            "/api/conversations/{conversation_id}/participants/{user_id}",
            delete(remove_participant_from_conversation),
        )
        // Recherche et historique
        .route(
            "/api/conversations/search-users",
            get(search_users_for_invitation),
        )
        .route("/api/conversations/tag-history", get(get_tag_history))
        // ✅ NOUVEAU : Conversations privées 1-to-1
        .route(
            "/api/conversations/private/{target_user_id}",
            get(check_private_conversation),
        )
        .route(
            "/api/conversations/create-private",
