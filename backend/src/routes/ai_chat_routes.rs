...
**How TrendPulse works (COMPLETE GUIDE):**\n\
TrendPulse (screen: TrendPulseDashboard) is Yukpo's AI-powered trend intelligence engine.\n\
HOW TO ACCESS TRENDPULSE:\n\
  Option A (direct): In YukpoIA chat → say 'tendances' or 'TrendPulse' → tap the purple button that appears\n\
  Option B (from SocialAI): Open SocialAI → tab 'Contenu' → scroll down → tap the TrendPulse banner\n\
  Option C (from Profil/Menu): Profile screen → 'Community Manager' section → TrendPulse\n\
  Option D (from YukpoIA quick actions): The purple 'Tendances' chip at the top of this chat\n\
WHAT TRENDPULSE SHOWS:\n\
- Trending topics list: score 0-100, direction arrows (↑rising ↓declining →stable), category badge\n\
- For each trend: D+3 / D+7 / D+14 forecast charts so you can post at peak timing\n\
- AI auto-drafts: YukpoIA generates a ready-to-publish post for each trending opportunity\n\
- Sector filter: topics are filtered by your business sector (fashion, food, tech, beauty, etc.)\n\
- Region filter: Cameroun / Sénégal / Côte d'Ivoire / Nigeria / global Africa\n\
RELATED SCREENS accessible from TrendPulseDashboard:\n\
- ReelScript (screen: ReelScript): generate a full TikTok/Reel/Shorts script from a trend\n\
  → hook, 5 scenes with duration+visual+text+action, CTA, hashtags, music vibe, 3 creator tips\n\
  → tap any trend → 'Créer un Reel' button → ReelScriptScreen pre-filled with that trend\n\
- SmartScheduling (screen: SmartScheduling): best time to post based on YOUR audience analytics\n\
  → heatmap 7 days × peak hours, top 5 slots ranked by predicted engagement\n\
  → accessible via: SocialAI → Contenu tab → 'Planification IA' button\n\
- BenchmarkAnalytics (screen: BenchmarkAnalytics): compare your performance vs sector average and top 25%\n\
  → metrics: engagement rate, reach, follower growth, posting frequency, response rate\n\
  → accessible via: SocialAI → tab Contenu → 'Benchmark' button, or SocialAnalyticsDashboard\n\
B2B API: businesses can subscribe to trend data via API key at /api/b2b/trends/{region} (TrendPulse B2B, screen: TrendPulseB2B)\n\n\
**How the Community Manager works (COMPLETE GUIDE):**\n\
The Community Manager (screen: SocialAI) is the full social media automation hub for sellers.\n\
HOW TO ACCESS COMMUNITY MANAGER:\n\
  → In YukpoIA chat: say 'community manager', 'chatbot', 'répondre automatiquement', 'inbox'\n\
  → Tab bar: SocialAI tab (the blue 'people' icon in the YukpoIA quick actions bar)\n\
  → Profile screen → 'Community Manager Yukpo'\n\
SocialAI has 5 TABS:\n\
1. Contenu — AI post generation (product → platform → tone → visual → generate → schedule)\n\
2. Chatbot — configure 24/7 auto-responder for Messenger/Instagram DM/WhatsApp/Comments\n\
3. Pubs — Meta ad campaigns (promo + retargeting DPA)\n\
4. Inbox — unified inbox all platforms (filters: Tous/Non lus/Escaladés/Étoilés/Commentaires)\n\
5. Calendrier — editorial calendar of scheduled and published posts\n\
ADVANCED MODULES (accessible via SocialAI or direct navigation):\n\
A) BRAND VOICE (screen: BrandVoice)\n\
  - Train YukpoIA on YOUR writing style: upload 5+ examples of your posts/messages\n\
  - Set: brand name, sector, tone (formal/casual/energetic), language (fr/en/lingala/wolof)\n\
  - Once trained, ALL generated content (posts, emails, chatbot replies) matches your brand voice\n\
  - Access: SocialAI → Contenu tab → 'Voix de marque' OR YukpoIA chat → say 'voix de marque'\n\
B) CONTENT APPROVAL (screen: ContentApproval)\n\
  - Review queue: all AI-generated posts waiting for human validation before publishing\n\
  - Approve/reject with one tap; add a comment for the AI to learn from\n\
  - Access: SocialAI → Contenu tab → 'Validation' OR YukpoIA chat → say 'valider contenu'\n\
C) LONGFORM CONTENT (screen: LongformContent)\n\
  - Generate long articles, YouTube scripts, newsletters, LinkedIn articles, podcast outlines\n\
  - Input: topic + content type + target audience → AI generates full structured document\n\
  - Access: SocialAI → Contenu tab → 'Contenu long format'\n\
D) ANALYTICS DASHBOARD (screen: SocialAnalyticsDashboard)\n\
  - Cross-platform dashboard: Instagram + Facebook + WhatsApp combined\n\
  - Metrics: total posts, engagement rate, reach, follower growth, best posts, ROAS\n\
  - Period: 7j / 30j / 90j\n\
  - A/B test results: which post variant performed better\n\
  - Export: download full CSV report (posts + customers + campaigns)\n\
  - Access: SocialAI → tab Analytics OR YukpoIA chat → say 'analytics'\n\
E) CRM LÉGER (screen: CRMLight)\n\
  - Customer profiles built automatically from WhatsApp/Messenger/Instagram conversations\n\
  - Segments: All / VIP / Follow-up needed / High value / Active last 7 days\n\
  - Per customer: conversation history, orders, VIP toggle, follow-up note, payment link button\n\
  - Access: SocialAI → Chatbot tab → 'CRM Clients' OR YukpoIA chat → say 'CRM' or 'clients'\n\
F) EMAIL MARKETING IA (screen: EmailCampaign)\n\
  - Create email campaigns: enter topic → YukpoIA generates subject + HTML email + text version\n\
  - Segments: all customers / VIP / high value / followed up\n\
  - Send via SendGrid to all opted-in customers with one tap\n\
  - Tracks: open rate, click rate, sent count\n\
  - Access: SocialAI → Contenu tab → 'Email Marketing' OR YukpoIA chat → say 'email campaign'\n\
G) WHATSAPP BROADCAST (screen: WhatsAppBroadcast)\n\
  - Send a WhatsApp message to ALL your customers at once (or by segment: VIP / active / etc.)\n\
  - Pre-built templates: promo flash, nouveautés, cadeau fidélité, annonce événement\n\
  - History tab: past broadcasts with delivery rate and sent count\n\
  - Recovery tab: detect abandoned carts and send automatic recovery messages\n\
  - Access: SocialAI → Chatbot tab → 'Broadcast WhatsApp' OR YukpoIA chat → say 'broadcast' or 'envoyer à tous'\n\
H) ABANDONED CART RECOVERY\n\
  - Automatically detects customers who showed purchase intent but did not complete order\n\
  - Sends a personalized WhatsApp recovery message within 2-48h\n\
  - Access: WhatsAppBroadcast screen → tab 'Recovery' → tap 'Détecter les paniers' then 'Relancer'\n\
I) MARKETPLACE CROSS-PARTENAIRES (screen: Marketplace)\n\
  - List your products to reach customers of OTHER Yukpo partners in your region\n\
  - Browse: filter by region (CM/SN/CI/NG) + category + text search\n\
  - Publish a listing: title, price, category, region, photo\n\
  - Access: SocialAI → Contenu tab → 'Marketplace' OR YukpoIA chat → say 'marketplace'\n\
J) REPUTATION MONITORING (screen: ReputationDashboard)\n\
  - Monitor all mentions of your brand across Facebook, Instagram, WhatsApp\n\
  - Crisis detection: auto-alerts when negative mentions spike\n\
  - Access: YukpoIA chat → say 'réputation' or 'mentions' or 'crise'\n\
K) ONBOARDING WIZARD (screen: OnboardingWizard)\n\
  - 3-step setup for new partners: connect social accounts → configure chatbot persona → generate first post\n\
  - Access: only shown to new partners who have not completed setup yet\n\
PAYMENT LINKS (WhatsApp):\n\
  - Generate a secure payment link and send it directly in a WhatsApp conversation\n\
  - Access: CRMLight screen → customer profile → 'Créer un lien de paiement'\n\n\
...
ENRICH PRODUCT WITH AI VISION:\n\
  - On any product in SocialAI or CRM: tap 'Enrichir avec IA' → send product photo\n\
  - YukpoIA (GPT-4o Vision) generates: professional description, SEO tags, suggested price\n\n\
Routes: SocialAI, TrendPulseDashboard, ReelScript, SmartScheduling, BenchmarkAnalytics, ReputationDashboard, BrandVoice, ContentApproval, LongformContent, SocialAnalyticsDashboard, SocialAccountSetup, CRMLight, EmailCampaign, WhatsAppBroadcast, Marketplace, OnboardingWizard\n\n\
