...
use crate::controllers::scheduling_search_controller::{
    get_available_medical_services, get_pharmacies_on_duty, refresh_pharmacies_on_duty,
...
pub fn scheduling_search_routes(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        // Pharmacies de garde
        .route(
            "/api/search/pharmacies-on-duty",
            get(get_pharmacies_on_duty),
        )
        // Services médicaux disponibles
        .route(
            "/api/search/medical-services",
            get(get_available_medical_services),
        )
        // Rafraîchissement de la vue matérialisée
        .route(
            "/api/admin/refresh-pharmacies",
            get(refresh_pharmacies_on_duty),
        )
        .with_state(state)
}
