...
pub async fn run_auto_migrations(pool: &PgPool) {
    ...
    // ✅ 2026-04-09 : WhatsApp Chatbot — sessions, abonnements alertes, alertes communautaires
    match ensure_whatsapp_chatbot_tables(pool).await {
        Ok(_) => info!("✅ Migration auto: WhatsApp chatbot tables (sessions, alertes) OK"),
        Err(e) => error!("❌ Erreur migration auto whatsapp_chatbot_tables: {}", e),
    }
...
/// ✅ 2026-04-09: Tables pour le chatbot WhatsApp Yukpo
/// - whatsapp_sessions : état de conversation par numéro
/// - whatsapp_alert_subscriptions : abonnés alertes par ville
/// - community_alerts : signalements GPS communautaires
pub async fn ensure_whatsapp_chatbot_tables(pool: &PgPool) -> Result<(), sqlx::Error> {
    // Sessions WhatsApp
    execute_migration_sql_safe(pool, r#"
