...
fn normalize_sql_command(cmd: &str) -> String {
    ...
    // Si DROP est déjà présent, laisser tel quel
    {
...
/// ✅ NOUVEAU: Vérifie et crée les tables pour le service Planification Menus
pub async fn ensure_menu_planning_tables(pool: &PgPool) -> Result<(), sqlx::Error> {
    info!("🔍 Vérification des tables menu planning...");
    ...
    sqlx::query(
        ...
        CREATE TABLE IF NOT EXISTS family_profiles (
            ...
            time_available_hours DECIMAL(4,2),
            created_at TIMESTAMPTZ DEFAULT NOW(),
            updated_at TIMESTAMPTZ DEFAULT NOW(),
            UNIQUE(user_id)
        )
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_menu_plans_user_week ON menu_plans(user_id, week_start)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_planned_meals_menu_plan ON planned_meals(menu_plan_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_shopping_lists_user_week ON shopping_lists(user_id, week_start)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_recipes_tags ON recipes USING GIN(tags)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_recipes_cuisine ON recipes(cuisine_style)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_shopping_list_items_list ON shopping_list_items(shopping_list_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_family_profiles_user ON family_profiles(user_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_recipe_favorites_user ON recipe_favorites(user_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_nutrition_analytics_user_week ON nutrition_analytics(user_id, week_start)")
        .execute(pool)
        .await?;

    info!("✅ Tables menu planning créées avec succès");
    Ok(())
}
