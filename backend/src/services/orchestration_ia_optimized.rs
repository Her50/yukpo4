...
pub async fn orchestrer_intention_ia_avec_optimisations(
    ...
    log_info(&format!(
        "[OrchestrationOptimized] ?? D?but orchestration optimis?e (ID: {})",
        interaction_id
    ));
    ...
    if let Some(cache) = &state.semantic_cache {
        ...
        log_info(&format!(
            "[OrchestrationOptimized] Recherche cache pour: {}",
            &cache_query[..50.min(cache_query.len())]
        ));
        ...
        match cache.get_smart(&cache_query, Some(&user_context)).await {
            Ok(Some(cached_response)) => {
                ...
                log_info(&format!(
                    "[OrchestrationOptimized] ?? CACHE HIT en {}?s - Performance Copilot!",
                    cache_time
                ));
                ...
                match serde_json::from_str::<Value>(&cached_response.content) {
                    Ok(mut cached_result) => {
                        ...
                        if let Some(obj) = cached_result.as_object_mut() {
                            ...
                            obj.insert(
                                "cached_confidence".to_string(),
                                json!(cached_response.confidence),
                            );
                        }

                        // ? Pr?diction en arri?re-plan pour futures requ?tes
                        if let Some(cache_ref) = &state.semantic_cache {
                            let cache_clone: std::sync::Arc<
                                crate::services::semantic_cache_pro::SemanticCachePro,
                            > = Arc::clone(cache_ref);
                            let context_clone = user_context.clone();
                            tokio::spawn(async move {
                                let _ = cache_clone
                                    .predict_and_precompute(&context_clone, &cache_query)
                                    .await;
                            });
                        }
                    ...
                    Err(e) => {
                        log_warn(&format!(
                            "[OrchestrationOptimized] Erreur d?codage cache: {}, fallback",
                            e
                        ));
            ...
            Err(e) => {
                log_warn(&format!(
                    "[OrchestrationOptimized] Erreur cache: {}, fallback",
                    e
                ));
                // Continue vers traitement normal
            }
    ...
    let optimized_prompt = if let Some(optimizer) = &state.prompt_optimizer {
        ...
        match optimizer.optimize_for_gpt4o(input, None).await {
            Ok(prompt) => {
                log_info(&format!(
                    "[OrchestrationOptimized] Prompt optimis?: {} chars",
                    prompt.len()
                ));
                prompt
            }
            Err(e) => {
                log_warn(&format!(
                    "[OrchestrationOptimized] Erreur optimisation prompt: {}, fallback",
                    e
                ));
    ...
    log_info(&format!(
        "[OrchestrationOptimized] IA response re?ue en {}ms depuis {}",
        ia_time, source
    ));

    // ? ?TAPE 4 : TRAITEMENT AVEC LOGIQUE EXISTANTE (R?utilise 100% l'existant)
    let result =
        match process_ia_response_with_existing_logic(&ia_response, &state, user_id, input).await {
            Ok(processed_result) => processed_result,
            Err(e) => {
                log_error(&format!(
                    "[OrchestrationOptimized] Erreur traitement r?ponse: {}",
                    e
                ));
    ...
    if let Some(cache) = &state.semantic_cache {
        ...
        match serde_json::to_string(&result) {
            Ok(result_string) => {
                    ...
                    .await
                {
                    Ok(()) => {
                        ...
                        log_info(&format!(
                            "[OrchestrationOptimized] R?sultat mis en cache en {}?s",
                            cache_store_time
                        ));
                    }
                    Err(e) => {
                        log_warn(&format!(
                            "[OrchestrationOptimized] Erreur stockage cache: {}",
                            e
                        ));
                    }
                }
            ...
            Err(e) => {
                log_warn(&format!(
                    "[OrchestrationOptimized] Erreur s?rialisation pour cache: {}",
                    e
                ));
            }
        }
    ...
    log_info(&format!(
        "[OrchestrationOptimized] ? Traitement optimis? termin? en {}ms (IA: {}ms)",
        total_time, ia_time
    ));
...
async fn process_ia_response_with_existing_logic(
    ...
    let mut data: Value = serde_json::from_str(&cleaned_response)
        .map_err(|e| format!("R?ponse IA non JSON: {} | {}", e, cleaned_response))?;
    ...
    if let Err(e) = orchestration_ia::valider_json_intention_ultra_avance(&intention, &data) {
        log_warn(&format!(
            "[OrchestrationOptimized] Validation ?chou?e: {}",
            e
        ));
        return Err(e);
    }
...
async fn route_to_existing_business_logic(
    ...
    log_info(&format!(
        "[OrchestrationOptimized] Routage m?tier pour intention: {}",
        intention
    ));
    ...
    match intention {
        "creation_service" => {
            ...
            .await
            .map(|(result, _)| result)
            .map_err(|e| format!("Erreur cr?ation service: {}", e).into())
        }

        "recherche_besoin" => {
            log_info("[OrchestrationOptimized] Route: recherche de besoin");
            crate::services::rechercher_besoin::rechercher_besoin(user_id, data)
                .await
                .map(|(result, _)| result)
                .map_err(|e| format!("Erreur recherche service: {}", e).into())
        }
        ...
        _ => {
            log_warn(&format!(
                "[OrchestrationOptimized] Intention non reconnue: {}",
                intention
            ));
            Ok(json!({
...
fn build_fallback_prompt(input: &MultiModalInput) -> String {
    ...
    prompt_parts.push(
        "Analyse la demande suivante et r?ponds au format JSON appropri? selon l'intention."
            .to_string(),
    );
    ...
    prompt_parts
        .push("R?ponds uniquement par un objet JSON valide sans texte additionnel.".to_string());
