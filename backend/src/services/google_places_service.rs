...
impl GooglePlacesService {
    ...
    pub async fn search_and_build_hierarchy(
        ...
        info!("🗺️ Recherche Google Places: {} ({})", place_name, country.unwrap_or("?"));
        ...
        if google_response.status != "OK" {
            if google_response.status == "ZERO_RESULTS" {
                ...
                return Err(AppError::NotFound(format!("Lieu '{}' introuvable", place_name)));
            } else {
                error!("❌ Google Places API erreur: {}", google_response.status);
                return Err(AppError::Internal(format!("Google Places API: {}", google_response.status)));
            }
        }
        
        // Prendre le premier résultat
        let place = google_response.results
            .into_iter()
            .next()
        ...
        for type_name in &type_order {
            if let Some(component) = place.address_components.iter().find(|c| c.types.contains(&type_name.to_string())) {
        ...
        info!("✅ Google Places: {} → {} éléments dans location_vector", place_name, location_vector.len());
        
        Ok(GooglePlacesResult {
            place_name: location_vector.first().cloned().unwrap_or_else(|| place_name.to_string()),
            location_vector,
...
mod tests {
    ...
    async fn test_search_makepe() {
        ...
        let result = service.search_and_build_hierarchy("Makepe", Some("Cameroun")).await;
