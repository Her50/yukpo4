...
impl GPUOptimizer {
    pub fn new() -> Self {
        ...
        log::info!(
            "[GPUOptimizer] Initialis? avec: {}",
            config.get_optimization_info()
        );
    ...
    pub async fn convert_all_modals_to_images_optimized(
        &self,
        ...
        if self.config.gpu_enabled {
            ...
            log::info!(
                "[GPUOptimizer] ? Conversion GPU termin?e en {:?}",
                start_time.elapsed()
            );
            result
        } else {
            ...
            log::info!(
                "[GPUOptimizer] ? Conversion CPU termin?e en {:?}",
                start_time.elapsed()
            );
            result
        }
    ...
    async fn convert_all_modals_to_images_gpu_parallel(
        &self,
        input: &MultiModalInput,
    ) -> Vec<String> {
    ...
    async fn process_images_optimized(
        &self,
    ...
    async fn process_excels_optimized(
        &self,
        excels: &Option<Vec<String>>,
    ) -> AppResult<Vec<String>> {
        if let Some(excel_list) = excels {
            log::info!(
                "[GPUOptimizer] Traitement de {} fichiers Excel",
                excel_list.len()
            );
    ...
    async fn optimize_single_image(
        img_base64: &str,
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            if width > _config.image_optimization.max_size
                || height > _config.image_optimization.max_size
            {
                image = image.resize(
    ...
    async fn convert_pdf_to_images_fallback(
        &self,
        pdf_base64: &str,
    ...
    async fn convert_excel_to_images_fallback(
        &self,
        excel_base64: &str,
    ) -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub fn get_stats(&self) -> String {
        format!(
            ...
            if self.config.image_optimization.parallel_processing {
                "YES"
            } else {
                "NO"
            }
