...
impl GlobalPromoService {
    ...
    pub async fn process_scheduler(state: Arc<AppState>) {
        ...
        loop {
            match Self::process_scheduler_inner(state.clone()).await {
                ...
                Err(err) => {
                    ...
                    if retry_count < MAX_RETRIES && is_connection_error {
                        ...
                        log::debug!(
                            "[GlobalPromo] ⚠️ Erreur connexion DB (retry {}/{}): {} - Attente {}ms",
                        ...
                        log::debug!(
                            "[GlobalPromo] ⚠️ Erreur connexion DB après {} retries (ignorée): {}",
