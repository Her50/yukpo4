...
impl ExhaustiveCombinationGenerator {
            ...
            .and_then(|s| s.as_array()) 
        {
            for dep in deps {
                ...
                let dims: Vec<String> = dep["dimensions"]
                    .as_array()
                    .ok_or_else(|| AppError::BadRequest("dimensions manquant dans dependency".to_string()))?
                    .iter()
                    .filter_map(|v| v.as_str())
                    .map(String::from)
                    .collect();
                
                let explanation = dep.get("explanation")
                    .and_then(|v| v.as_str())
                    .map(String::from);
    ...
    fn generate_with_fixed_dependent(
        ...
        let independent_values: Vec<Vec<String>> = independent_dims.iter()
