...
pub async fn generate_timeline_variants(
    ...
    for (index, style_name) in selected_styles.iter().take(variant_count).enumerate() {
        ...
        let mut variant_request = request.base_request.clone();
        ...
        // Générer la timeline avec ce style
        match app_ia.generate_video_timeline(&variant_request).await {
            Ok(timeline) => {
                ...
                info!("[TimelineVariant] ✅ Variante '{}' générée", style_name);
            }
            Err(e) => {
                error!(
