...
impl VideoTranscodingService {
    ...
    pub async fn initialize(&self) -> AppResult<()> {
        ...
        fs::create_dir_all(&self.output_dir).await
            .map_err(|e| AppError::Internal(format!("Erreur création dossier transcodage: {}", e)))?;
    ...
    pub async fn transcode_video(&self, video_path: &str, video_id: i32) -> AppResult<TranscodedVideo> {
    ...
    async fn do_transcode(&self, video_path: &str, video_id: i32) -> AppResult<TranscodedVideo> {
        log_info(&format!("[VideoTranscoding] Début transcodage vidéo ID: {}", video_id));
        ...
        let qualities = vec![
            VideoQuality { resolution: "1920x1080".to_string(), bitrate: 5000, fps: 30, label: "1080p".to_string() },
            VideoQuality { resolution: "1280x720".to_string(), bitrate: 2500, fps: 30, label: "720p".to_string() },
            VideoQuality { resolution: "854x480".to_string(), bitrate: 1000, fps: 30, label: "480p".to_string() },
            VideoQuality { resolution: "640x360".to_string(), bitrate: 500, fps: 30, label: "360p".to_string() },
        ];
        ...
        log_info(&format!("[VideoTranscoding] Transcodage terminé pour vidéo ID: {}", video_id));
        Ok(transcoded)
    }
    ...
    async fn generate_thumbnail(&self, video_path: &str, output_base: &Path) -> AppResult<String> {
        ...
        if !output.status.success() {
            ...
            return Err(AppError::Internal(format!("FFmpeg thumbnail error: {}", stderr)));
        }
    ...
    async fn transcode_to_hls(&self, video_path: &str, output_base: &Path, qualities: &[VideoQuality]) -> AppResult<String> {
        ...
        for quality in qualities {
            let quality_name = format!("{}_{}", video_base.file_stem().unwrap().to_str().unwrap(), quality.label);
            let variant_path = output_base.join(&quality_name);
            fs::create_dir_all(&variant_path).await
                .map_err(|e| AppError::Internal(format!("Erreur création dossier qualité: {}", e)))?;
            ...
            let output = Command::new(&self.ffmpeg_path)
                .args(&[
                    "-i", video_path,
                    "-vf", &format!("scale={}:force_original_aspect_ratio=decrease", quality.resolution),
                    "-b:v", &format!("{}k", quality.bitrate),
                    "-r", &quality.fps.to_string(),
                    "-c:v", "libx264",
                    "-c:a", "aac",
                    "-b:a", "128k",
                    "-f", "hls",
                    "-hls_time", "6",
                    "-hls_list_size", "0",
                    "-hls_segment_filename", &variant_path.join(&segment_filename).to_str().unwrap(),
                    "-y",
                    playlist_path.to_str().unwrap()
                ])
                .output()
                .await
                .map_err(|e| AppError::Internal(format!("Erreur transcodage HLS {}: {}", quality.label, e)))?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                log_error(&format!("[VideoTranscoding] Erreur HLS {}: {}", quality.label, stderr));
                continue;
            }
        ...
        fs::write(&master_playlist, master_content).await
    ...
    async fn transcode_to_dash(&self, video_path: &str, output_base: &Path, qualities: &[VideoQuality]) -> AppResult<String> {
        ...
        let mut args = vec![
            ...
            "-adaptation_sets".to_string(), "id=0,streams=v".to_string(),
            "-y".to_string(),
        ];
        ...
        for (i, quality) in qualities.iter().enumerate() {
            args.extend(vec![
                ...
                "-b:v:0".to_string(), format!("{}k", quality.bitrate),
                "-vf:0".to_string(), format!("scale={}:force_original_aspect_ratio=decrease", quality.resolution),
            ]);
        }
    ...
    async fn get_video_metadata(&self, video_path: &str) -> AppResult<VideoMetadata> {
        ...
        if !output.status.success() {
            return Err(AppError::Internal("Impossible d'extraire les métadonnées vidéo".to_string()));
        }
    ...
    async fn save_transcoded_metadata(&self, video_id: i32, transcoded: &TranscodedVideo) -> AppResult<()> {
    ...
    pub async fn get_transcoded_urls(&self, video_id: i32) -> AppResult<Option<TranscodedVideo>> {
        ...
        if let Some(row) = row {
            let qualities: Vec<VideoQuality> = serde_json::from_str(&row.qualities)
                .map_err(|e| AppError::Internal(format!("Erreur désérialisation qualités: {}", e)))?;
