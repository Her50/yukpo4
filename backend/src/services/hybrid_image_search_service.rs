...
impl HybridImageSearchService {
    ...
    async fn analyze_image_like_creation(
        ...
        let search_prompt = match tokio::fs::read_to_string(
            "backend/ia_prompts/recherche_image_prompt_optimized.md",
        )
        .await
        {
            Ok(content) => {
                log_info(
                    "[HybridImageSearch] ✅ Prompt optimisé chargé depuis fichier (~100 lignes)",
                );
                content
            }
            Err(_) => {
        ...
        let parsed_json: serde_json::Value = serde_json::from_str(&cleaned_json).map_err(|e| {
            log_error(&format!(
                "[HybridImageSearch] ❌ Erreur parsing JSON brut: {}",
                e
            ));
            log_error(&format!(
                "[HybridImageSearch] JSON reçu (premiers 500 chars): {}",
                &cleaned_json.chars().take(500).collect::<String>()
            ));
        ...
        if let Err(e) = Self::validate_ia_json(data_obj) {
            log_error(&format!(
                "[HybridImageSearch] ❌ Validation JSON échouée: {}",
                e
            ));
        ...
        let description_produit = data_obj
        ...
        if let Some(prod_obj) = produits_autocomplete.and_then(|p| p.as_object()) {
                ...
                .and_then(|sc| sc.as_object())
            {
                ...
                // Marque
                if let Some(marques_arr) = sous_caracs
                    .get("marque")
                ...
                // Modèle
                if let Some(modeles_arr) = sous_caracs
                    .get("modele")
                ...
                // Couleurs
                if let Some(couleurs_arr) = sous_caracs
                    .get("couleur")
                    .or_else(|| sous_caracs.get("color"))
                ...
                if !description_produit.is_empty() {
                    ...
                    for word in words.iter().take(10) {
                        // Limiter à 10 mots pour éviter trop de tags
    ...
    pub async fn search_by_image(
        ...
        let (analysis, cost) = match analysis_result {
            ...
            Err(e) => {
                ...
                return self
                    .search_by_image_signature_fallback(
                        image_base64,
                        category_filter,
                        gps_lat,
                        gps_lng,
    ...
    async fn search_by_image_signature_fallback(
        ...
        let hybrid_results: Vec<HybridSearchResult> = signature_results
            ...
            .map(|sr| {
                ...
                let product_tags: Vec<String> = sr
                    .service_data
                    .get("tags")
        ...
        let fallback_analysis = ImageAnalysis {
            description: format!(
                "Recherche par similarité visuelle (fallback - {} résultats)",
                hybrid_results.len()
            ),
