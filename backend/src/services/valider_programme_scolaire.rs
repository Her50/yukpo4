...
pub fn valider_programme_scolaire_json(data: &Value) -> Result<(), AppError> {
    ...
    let schema_str =
        fs::read_to_string("src/schemas/programme_scolaire_schema.json").map_err(|e| {
            AppError::Internal(format!("Erreur lecture sch?ma programme_scolaire: {e}"))
        })?;
    let schema_json: Value = serde_json::from_str(&schema_str).map_err(|e| {
        AppError::Internal(format!("Erreur parsing sch?ma programme_scolaire: {e}"))
    })?;
    let compiled = JSONSchema::compile(&schema_json).map_err(|e| {
        AppError::Internal(format!("Erreur compilation sch?ma programme_scolaire: {e}"))
    })?;
