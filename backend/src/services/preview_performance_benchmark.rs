...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_timeline = VideoTimeline {
        total_duration: 30.0,
        scenes: vec![
            crate::services::app_ia::TimelineScene {
                scene_index: 0,
                start_time: 0.0,
                duration: 5.0,
                ...
                effects: Some(vec!["blur".to_string()]),
                ..Default::default()
            },
            crate::services::app_ia::TimelineScene {
                scene_index: 1,
    ...
    for i in 1..=iterations {
        ...
        match generate_quick_preview(request.clone(), None).await {
            Ok(response) => {
                ...
                println!("Itération {}: {}ms (backend: {}ms)", 
                         i, elapsed_ms, response.processing_time_ms);
                
                if elapsed_ms < 100 {
                    println!("  ✅ Performance < 100ms");
                } else {
                    println!("  ⚠️  Performance > 100ms ({}ms au-dessus)", elapsed_ms - 100);
                }
            }
            Err(e) => {
    ...
    if !times.is_empty() {
        ...
        if avg < 100.0 {
            ...
            println!("⚠️  Performance moyenne > 100ms ({:.2}ms, {}ms au-dessus)", avg, avg - 100.0);
        }
    }
