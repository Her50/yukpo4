...
pub async fn check_all_tables(
    ...
    for table_name in tables {
        ...
        let exists = sqlx::query_scalar::<_, bool>(
            &format!("SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = '{}')", table_name)
        )
        .fetch_one(pool)
        .await
        .unwrap_or(false);
    ...
    for func_name in functions {
        let exists = sqlx::query_scalar::<_, bool>(
            &format!("SELECT EXISTS(SELECT 1 FROM pg_proc WHERE proname = '{}')", func_name)
        )
        .fetch_one(pool)
        .await
        .unwrap_or(false);
    ...
    if results.get("african_locations").and_then(|v| v.get("exists")).and_then(|v| v.as_bool()).unwrap_or(false) {
    ...
    if results.get("autocomplete_characteristics").and_then(|v| v.get("exists")).and_then(|v| v.as_bool()).unwrap_or(false) {
    ...
    if results.get("autocomplete_combinations").and_then(|v| v.get("exists")).and_then(|v| v.as_bool()).unwrap_or(false) {
        let combinations_count = sqlx::query_scalar::<_, i64>(
    ...
    log::info!("✅ [Debug] Vérification terminée: {}/{} tables existent", total_tables_exist, total_tables_exist + total_tables_missing);
...
pub async fn check_autocomplete_tables(
    ...
    if geo_exists {
        ...
        results.insert("geo_hierarchy".to_string(), json!({
            "exists": true,
            "count": count,
    ...
    if african_exists {
        ...
        results.insert("african_locations".to_string(), json!({
            "exists": true,
            "total_count": count,
            "villes": villes_map,
    ...
    if char_exists {
        let total = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM autocomplete_characteristics")
            .fetch_one(pool)
            .await
            .unwrap_or(0);
        ...
        results.insert("autocomplete_characteristics".to_string(), json!({
            "exists": true,
            "total_count": total,
    ...
    if comb_exists {
        ...
        results.insert("autocomplete_combinations".to_string(), json!({
            "exists": true,
            "count": count,
    ...
    if history_exists {
        ...
        results.insert("search_history".to_string(), json!({
            "exists": true,
            "count": count,
