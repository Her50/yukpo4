...
use crate::services::creer_service::clean_media_recursive_final;
...
pub async fn process_product_creation(
    ...
    log_info(&format!(
        "[process_product_creation] 🔄 Traitement produit pour service {} (user_id: {})",
        service_id, user_id
    ));
    ...
    log::info!(
        ...
        if product_description_after.is_empty() {
            "ABSENTE"
        } else {
    ...
    let product_result = products_service
        .create_product(service_id, product_index, &product_data_cleaned)
        .await;
    ...
    match product_result {
        Ok(product) => {
            ...
            let service_data: Option<serde_json::Value> =
...
pub async fn add_product_to_service(
    ...
    log_info(&format!(
        "[add_product_to_service] 📦 Ajout d'un produit au service {} (user_id: {})",
        service_id, user.id
    ));
