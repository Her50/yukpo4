...
pub async fn get_services_list(State(_state): State<Arc<AppState>>) -> axum::response::Response {
    (
...
pub async fn get_services_recent(State(_state): State<Arc<AppState>>) -> axum::response::Response {
    (
        StatusCode::NOT_IMPLEMENTED,
        Json(json!({
...
pub async fn modifier_service(
    ...
    let service_exists = sqlx::query("SELECT id FROM services WHERE id = $1 AND user_id = $2")
        .bind(service_id)
        .bind(user_id)
    ...
    let result = if payload_keys.len() == 1 && payload_keys.contains(&"produits".to_string()) {
        if let Some(produits_value) = payload.data.get("produits") {
            info!(
                "[modifier_service] 🚀 Mise à jour partielle (produits uniquement) pour service {}",
                service_id
            );
            sqlx::query(
                r#"
            ...
            info!(
                "[modifier_service] 📝 Mise à jour complète du service {} (fallback)",
                service_id
            );
            sqlx::query(
                r#"
        ...
        info!(
            "[modifier_service] 📝 Mise à jour complète du service {} ({} champs)",
            service_id,
            payload_keys.len()
        );
        sqlx::query(
...
pub async fn supprimer_service(
    ...
    let service_data = sqlx::query("SELECT id, data FROM services WHERE id = $1 AND user_id = $2")
        .bind(service_id)
        .bind(user_id)
        .fetch_optional(pg_pool)
        .await;
    ...
    let result = sqlx::query("DELETE FROM services WHERE id = $1 AND user_id = $2 RETURNING id")
        .bind(service_id)
        .bind(user_id)
        .fetch_optional(pg_pool)
        .await;
            ...
            match products_service.get_products_by_service(service_id).await {
                Ok(products) => {
                    if !products.is_empty() {
                        info!(
                            "[get_service_by_id] {} produits trouvés pour le service {}",
                            products.len(),
                            service_id
                        );
                        ...
                        info!(
                            "[get_service_by_id] Aucun produit trouvé pour le service {}",
                            service_id
                        );
                    }
                }
                Err(e) => {
                    warn!(
                        "[get_service_by_id] Erreur lors du chargement des produits: {}",
                        e
                    );
...
pub async fn get_services_batch(
    ...
    info!(
        "[get_services_batch] Récupération de {} services",
        service_ids.len()
    );
    ...
    info!(
        "[get_services_batch] {} services trouvés sur {} demandés",
        result.len(),
        service_ids.len()
    );
...
pub async fn get_services_by_user_id(
    ...
    info!(
        "[get_services_by_user_id] Récupération des services pour utilisateur {}",
        user_id
    );
    ...
    info!(
        "[get_services_by_user_id] {} services trouvés pour utilisateur {}",
        rows.len(),
        user_id
    );
...
pub async fn get_shared_service(
    ...
    let prestataire_info =
        match sqlx::query(r#"SELECT nom, prenom, photo_profil FROM users WHERE id = $1"#)
