...
pub async fn validate_partner(
    ...
    if action != "approve" && action != "reject" {
        return Err(AppError::BadRequest(
            "action doit être 'approve' ou 'reject'".into(),
        ));
    }
    ...
    if action == "approve" {
        ...
        if email_service.is_available() {
            ...
            match email_service
                .send_simple_email(&partner_info.email, email_subject, &email_body)
                .await
            {
                Ok(result) => {
                    if result.success {
                        log::info!(
                            "[validate_partner] ✅ Email d'approbation envoyé à {}",
                            partner_info.email
                        );
                    } else {
                        warn!(
                            "[validate_partner] ⚠️ Échec envoi email: {:?}",
                            result.error
                        );
                    }
                }
        ...
        // Rejeter
        sqlx::query("UPDATE users SET partner_status = 'rejected' WHERE id = $1")
            .bind(user_id)
            .execute(&state.pg)
            .await?;
        ...
        {
            warn!(
                "[validate_partner] ⚠️ Impossible de créer la notification: {}",
                e
            );
        } else {
        ...
        {
            warn!(
                "[validate_partner] ⚠️ Impossible d'envoyer la push notification: {}",
                e
            );
        } else {
            log::info!("[validate_partner] ✅ Push notification envoyée");
        ...
        if email_service.is_available() {
                ...
                .await
            {
                Ok(result) => {
                    if result.success {
                        log::info!(
                            "[validate_partner] ✅ Email de rejet envoyé à {}",
                            partner_info.email
                        );
                    } else {
                        warn!(
        ...
        if let Some((lat_str, lng_str)) = gps_str.split_once(',') {
                ...
                (lat_str.trim().parse::<f64>(), lng_str.trim().parse::<f64>())
            {
                ...
                match geocoding_service.reverse_geocode(lat, lng).await {
                    ...
                    Err(e) => {
                        warn!(
                            "[get_user_country] Erreur géocodage inverse pour user_id={}: {}",
                            user_id, e
                        );
                    }
                }
