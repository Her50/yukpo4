...
pub async fn deactivate_product(
    ...
    log_info(&format!("[deactivate_product] ✅ Produit {} désactivé avec succès", product_index));
...
pub async fn reactivate_product(
    ...
    if current_balance < cost {
        log_error(&format!("[reactivate_product] Solde insuffisant: {} < {}", current_balance, cost));
    ...
    let _ = crate::services::notification_service::create_notification(
        ...
        format!("Votre produit '{}' a été réactivé avec succès. Coût: {} FCFA.", product_name, cost),
        Some(json!({
            "service_id": service_id,
            "product_index": product_index,
            "product_name": product_name,
    ...
    log_info(&format!("[reactivate_product] ✅ Produit {} réactivé (coût: {} FCFA)", product_index, cost));
