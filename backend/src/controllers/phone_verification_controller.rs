...
pub async fn request_phone_verification(
    ...
    sqlx::query("UPDATE phone_verification_codes SET is_used = true WHERE user_id = $1")
        ...
        .map_err(|e| {
            log::error!(
                "[request_phone_verification] Erreur nettoyage anciens codes: {}",
                e
            );
    ...
    if let Err(e) =
        send_verification_code(&payload.phone, &payload.country, &otp_code, &state).await
    {
    ...
    log::info!(
        "[request_phone_verification] Code envoyé pour utilisateur {}: {}",
        user.id,
        payload.phone
    );
...
pub async fn verify_phone_code(
    ...
    if code_record.attempts >= 3 {
        return Err(AppError::BadRequest(
            "Trop de tentatives. Veuillez demander un nouveau code.".to_string(),
        ));
    }
    ...
    log::info!(
        "[verify_phone_code] Téléphone vérifié pour utilisateur {}: {}",
        user.id,
        code_record.phone
    );
...
async fn send_verification_code(
    ...
    if can_receive_whatsapp(&Some(phone.to_string()), &Some(country.to_string())) {
        if let Err(e) = send_whatsapp_message(&formatted_phone, &message, state).await {
            log::warn!(
                "[send_verification_code] WhatsApp échoué, fallback SMS: {}",
                e
            );
            // Fallback vers SMS
...
async fn send_whatsapp_message(
    ...
    let _account_sid =
        std::env::var("TWILIO_ACCOUNT_SID").map_err(|_| "TWILIO_ACCOUNT_SID non configuré")?;
    let _auth_token =
        std::env::var("TWILIO_AUTH_TOKEN").map_err(|_| "TWILIO_AUTH_TOKEN non configuré")?;
...
async fn send_sms_message(
    ...
    let _auth_token =
        std::env::var("TWILIO_AUTH_TOKEN").map_err(|_| "TWILIO_AUTH_TOKEN non configuré")?;
    let from_number =
        std::env::var("TWILIO_SMS_NUMBER").unwrap_or_else(|_| "+1234567890".to_string());

    log::info!(
        "[send_sms_message] SMS {} -> {}: {}",
        from_number,
        phone,
        message
    );
...
fn can_receive_whatsapp(phone: &Option<String>, country: &Option<String>) -> bool {
    ...
    let whatsapp_countries = [
        "CM", "CI", "SN", "CD", "GA", "TG", "BJ", "BF", "ML", "NE", "TD",
    ];
