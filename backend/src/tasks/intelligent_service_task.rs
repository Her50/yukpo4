...
pub async fn run_intelligent_service_processing(pool: &PgPool) {
    ...
    match process_expired_services_intelligently(pool).await {
        Ok(result) => {
            if result.auto_renewed > 0
                || result.manually_deactivated > 0
                || result.tarissable_deactivated > 0
            {
                info!("🎯 Traitement intelligent terminé:");
                info!(
                    "  ✅ Services renouvelés automatiquement: {}",
                    result.auto_renewed
                );
                info!(
                    "  ❌ Services désactivés (solde insuffisant): {}",
                    result.manually_deactivated
                );
                info!(
                    "  🍎 Services tarissables désactivés: {}",
                    result.tarissable_deactivated
                );
        ...
        Err(e) => {
            error!(
                "❌ Erreur lors du traitement intelligent des services: {}",
                e
            );
        }
    }
...
pub async fn run_complete_service_processing(pool: &PgPool) {
    ...
    match crate::tasks::product_deactivation::deactivate_expired_products(pool).await {
        Ok(count) => info!(
            "✅ Désactivation automatique: {} produits désactivés",
            count
        ),
