...
pub async fn start_publicite_expiration_task(pool: Arc<PgPool>) {
    tracing::info!("🕐 [Cron] Démarrage de la tâche de désactivation des publicités expirées");
    ...
    loop {
        ...
        tracing::info!("⏰ [Cron] Vérification des publicités expirées...");

        match check_and_deactivate_expired_publicites(&pool).await {
            Ok(count) => {
                if count > 0 {
                    tracing::info!("✅ [Cron] {} publicité(s) expirée(s) désactivée(s)", count);
                } else {
                    tracing::debug!("✓ [Cron] Aucune publicité expirée trouvée");
                }
            }
            Err(e) => {
                tracing::error!("❌ [Cron] Erreur lors de la désactivation: {:?}", e);
            }
        }
        ...
        match check_expiring_soon_publicites(&pool).await {
            Ok(count) => {
                if count > 0 {
                    tracing::info!("📧 [Cron] {} notification(s) envoyée(s) pour publicités proches expiration", count);
                }
            }
            Err(e) => {
                tracing::error!("❌ [Cron] Erreur notifications expiration: {:?}", e);
            }
        }
    }
}
...
async fn check_expiring_soon_publicites(pool: &PgPool) -> Result<u64, sqlx::Error> {
    ...
    for pub_record in expiring_publicites {
        ...
        .await
        {
            ...
            Err(e) => {
                tracing::warn!("Erreur création notification: {:?}", e);
            }
        }
    }
...
pub async fn manual_deactivate_expired_publicites(pool: &PgPool) -> Result<u64, sqlx::Error> {
    tracing::info!("🔧 [Manual] Désactivation manuelle des publicités expirées");
