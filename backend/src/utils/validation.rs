...
pub fn validate_email(email: &str) -> Result<(), AppError> {
    ...
    if email.len() > 255 {
        return Err(AppError::BadRequest(
            "L'email est trop long (max 255 caractères)".into(),
        ));
    }
...
pub fn validate_text_length(
    ...
    if len < min {
        return Err(AppError::BadRequest(format!(
            "{} doit contenir au moins {} caractères",
            field_name, min
        )));
    }
...
pub fn sanitize_string(input: &str, max_length: usize) -> Result<String, AppError> {
    ...
    if cleaned.len() > max_length {
        return Err(AppError::BadRequest(format!(
            "Le champ est trop long (max {} caractères)",
            max_length
        )));
    }
    ...
    if cleaned.contains('\0') {
        return Err(AppError::BadRequest(
            "Caractères non autorisés détectés".into(),
        ));
    }
