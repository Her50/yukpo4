...
pub async fn hide_headers(req: Request<Body>, next: Next) -> Response {
    ...
    if let Ok(x_content_type_options) = HeaderValue::from_static("nosniff") {
    ...
    if let Ok(x_frame_options) = HeaderValue::from_static("DENY") {
        res.headers_mut().insert(
    ...
    if req.uri().scheme_str() == Some("https") {
        if let Ok(hsts) = HeaderValue::from_static("max-age=31536000; includeSubDomains") {
            res.headers_mut().insert(
    ...
    if let Ok(x_xss_protection) = HeaderValue::from_static("1; mode=block") {
        res.headers_mut().insert(
    ...
    if let Ok(referrer_policy) = HeaderValue::from_static("strict-origin-when-cross-origin") {
        res.headers_mut().insert(
    ...
    if let Ok(permissions_policy) = HeaderValue::from_static(
