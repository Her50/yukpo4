...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let migration_sql = include_str!("../../migrations/20260104_apply_delivery_partners_migrations.sql");
        ...
        for line in sql.lines() {
            ...
            if trimmed.contains("$$") {
                if !in_dollar_quote {
                    ...
                    // Fin d'un bloc $$
                    if trimmed.contains(&format!("$${}", dollar_tag)) || (dollar_tag.is_empty() && trimmed.ends_with("$$")) {
    ...
    match execute_multiple_sql_commands(&pool, migration_sql).await {
        ...
        Err(e) => {
            ...
            if error_msg.contains("already exists") 
                ...
                || error_msg.contains("does not exist")
                || error_msg.contains("already has value") {
                println!("ℹ️  Migration partiellement appliquée (certaines étapes déjà effectuées)");
            } else {
