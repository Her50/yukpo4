// Script de test Redis standalone - ne dépend pas du reste du projet
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let redis_url = env::var("REDIS_URL").unwrap_or_else(|_| {
        ...
        println!("💡 Utilisez: $env:REDIS_URL=\"rediss://...\"");
        std::process::exit(1);
    });
    ...
    match client.get_multiplexed_async_connection().await {
        Ok(mut conn) => {
            ...
            match deadpool_redis::Config::from_url(normalized_url.clone()) {
                mut cfg => {
                    cfg.max_size = Some(4);
                    cfg.min_idle = Some(1);
                    match cfg.create_pool(Some(deadpool_redis::Runtime::Tokio1)) {
                        Ok(pool) => {
                            ...
                            match pool.get().await {
                                ...
                                Err(e) => {
                                    println!("   ⚠️  Pool créé mais impossible d'obtenir une connexion: {}", e);
                                }
                            }
                        }
                        Err(e) => {
