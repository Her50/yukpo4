# Script pour verifier la vue delivery_requests

param(
    [string]$Region = "us-east-1"
)

Write-Host "Verification de la vue delivery_requests..." -ForegroundColor Cyan
Write-Host ""
...
$verificationSQL = "SELECT CASE WHEN EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'delivery_requests' AND schemaname = 'public') THEN 'EXISTS' ELSE 'MISSING' END as delivery_requests_view, CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'courier_profiles') THEN 'EXISTS' ELSE 'MISSING' END as courier_profiles_table;"
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $taskDefArn -notmatch "error") {
        ...
        if ($LASTEXITCODE -eq 0 -and $taskArn -notmatch "error" -and $taskArn.Length -gt 0) {
            ...
            if ($logs) {
                $result = ($logs | Select-Object -Last 1).Trim()
                $parts = $result -split '\|'
                if ($parts.Length -ge 2) {
                    $viewStatus = $parts[0].Trim()
                    $tableStatus = $parts[1].Trim()
                    
                    Write-Host "Resultats:" -ForegroundColor Yellow
                    Write-Host "  Vue delivery_requests: $viewStatus" -ForegroundColor $(if ($viewStatus -eq 'EXISTS') { 'Green' } else { 'Red' })
                    Write-Host "  Table courier_profiles: $tableStatus" -ForegroundColor $(if ($tableStatus -eq 'EXISTS') { 'Green' } else { 'Red' })
                    Write-Host ""
                    
                    if ($viewStatus -eq 'EXISTS' -and $tableStatus -eq 'EXISTS') {
                        Write-Host "✅ Toutes les migrations restantes ont ete appliquees avec succes!" -ForegroundColor Green
                    }
                }
            }
        }
