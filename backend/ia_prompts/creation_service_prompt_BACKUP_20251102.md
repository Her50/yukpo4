...
### 🧮 LOGIQUE D'ARRANGEMENTS COMBINATOIRES (NOUVEAU 2025-11-02)

**Quand générer multi-combinaisons** :
- ✅ **Avec IMAGE précise** → 1 seule combinaison (produit identifié)
- ✅ **Avec TEXTE vague** → Plusieurs combinaisons (arrangements logiques)

**PROCESSUS EN 3 ÉTAPES :**

#### **ÉTAPE 1 : Construire les modalités par sous-caractéristique**

Pour chaque dimension, lister TOUTES les valeurs possibles courantes.

Exemple "je vends du riz" :
```
type: [Riz] = 1 option (fixe)
variete: [Basmati, Jasmin, Thaï, Parboiled, Complet, Uncle Ben's, Taureau, Mémé Cassé] = 8 options
poids: [1kg, 5kg, 10kg, 25kg, 50kg] = 5 options
couleur: [Blanc, Brun, Rouge] = 3 options
qualite: [Premium, Standard, Économique] = 3 options
origine: [Inde, Thaïlande, Local, Pakistan, Vietnam] = 5 options
conditionnement: [Sac, Paquet, Vrac] = 3 options
lieu: [""] = 1 option (vide, rempli par prestataire)
```

**Total combinaisons possibles** = 1 × 8 × 5 × 3 × 3 × 5 × 3 = **5 400 combinaisons** !

#### **ÉTAPE 2 : Sélectionner des arrangements LOGIQUES**

**❌ NE PAS faire** : Fixer arbitrairement certaines valeurs
- ❌ Toutes avec 5kg (poids fixe) - PAS DE VARIÉTÉ !
- ❌ Toutes blanches - PAS DE VARIÉTÉ !

**✅ FAIRE** : Arrangements intelligents avec VRAIE variété

**Stratégie 1 : Varier la dimension PRINCIPALE (variété de riz)**
```
Riz,Basmati,5kg,Blanc,Premium,Inde,Sac,          // Basmati populaire
Riz,Jasmin,5kg,Blanc,Premium,Thaïlande,Sac,      // Jasmin populaire
Riz,Uncle Ben's,1kg,Blanc,Standard,USA,Paquet,   // Uncle Ben's petit format
Riz,Taureau,25kg,Blanc,Économique,Local,Sac,     // Taureau gros format économique
Riz,Complet,1kg,Brun,Premium,Bio,Paquet,         // Complet pour santé
```

**Stratégie 2 : Varier 2 dimensions (variété + poids)**
```
Riz,Basmati,1kg,Blanc,Premium,Inde,Paquet,
Riz,Basmati,5kg,Blanc,Premium,Inde,Sac,
Riz,Jasmin,10kg,Blanc,Premium,Thaïlande,Sac,
Riz,Uncle Ben's,1kg,Blanc,Standard,USA,Paquet,
Riz,Taureau,25kg,Blanc,Économique,Local,Sac,
```

**Stratégie 3 : TOP combinaisons POPULAIRES (connaissance du marché)**
```
Riz,Taureau,25kg,Blanc,Économique,Local,Sac,     // #1 Afrique - Gros sac économique
Riz,Uncle Ben's,1kg,Blanc,Standard,USA,Paquet,   // #2 Petit format pratique
Riz,Basmati,5kg,Blanc,Premium,Inde,Sac,          // #3 Premium moyen format
Riz,Jasmin,5kg,Blanc,Premium,Thaïlande,Sac,      // #4 Alternative premium
Riz,Parboiled,10kg,Blanc,Standard,Local,Sac,     // #5 Gros format local
```

#### **ÉTAPE 3 : Marquer la combinaison PRÉFÉRÉE**

- Si caractéristiques **explicites** dans le texte → Position 0
- Si texte **vague** → Combinaison la plus **populaire/logique** en position 0
...
1. **Image précise** → 1 combinaison, pas de `ai_preferred_index`
2. **Texte vague** → 5-15 combinaisons VARIÉES, `ai_preferred_index: 0` OBLIGATOIRE
3. **Arrangements** → Varier intelligemment 1-2 dimensions principales
4. **Éviter** : Fixer les mêmes valeurs pour toutes les combinaisons
5. **Prioriser** : Combinaisons populaires/courantes dans la région
